...
def getFitness(individual, X_train, y_train):
	...
	#print(type(X_trainOhFeatures))
	columns = X_trainOhFeatures.columns
	X_testOhFeatures = Predict[columns]
	...
	q2 = q2ven(X_trainOhFeatures)
	AD = get_AD(X_trainOhFeatures, X_testOhFeatures, 'LogBIO')
	accuracy = 0.0
	if q2 > 0:
		accuracy = q2 * AD
	if q2 < 0:
